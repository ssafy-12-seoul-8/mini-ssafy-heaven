<template>
    <div>
      <div class="flex items-center justify-center">
        <div class="relative">
          <input
            class="border-b border-gray-300 py-1 focus:border-b-2 focus:border-blue-400 transition-colors focus:outline-none peer bg-inherit text-3xl"
            :id="name"
            :name="name"
            :type="type"
            placeholder=""
...
const type = ref('text')
  const { name, label, password, modelValue } = defineProps({
    name: {
      type: String,
      required: true },
    label: {
